...
async function LoadScripts(e) {
  if (1 === e.length) {
    ...
    const i = e.map((t) => `import "${IsRelativeURL(t) ? "./" : ""}${t}";`).join("\n"),
      r = URL.createObjectURL(new Blob([i], { type: "application/javascript" }));
    try {
      await import(r);
    } catch (t) {
      console.warn(
...
async function InitRuntime(t) {
  ...
  (t["runOnStartupFunctions"] = r), e.postMessage({ type: "creating-runtime" }), (runtime = self["C3_CreateRuntime"](t)), await self["C3_InitRuntime"](runtime, t);
}
